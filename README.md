...
视频处理任务通过 Kafka 异步调度，处理阶段和结果落库到 MySQL；MinIO 负责对象存储，Milvus 与关键词检索共同支撑视频 RAG 问答。Vue 界面主要用于验证和展示后端能力。
...
RAG 评测工具位于 `tools/rag_eval/`，用于固定数据集、检索结果和评测产物，支持对比不同检索配置。评测数据格式和标注规则见 `docs/eval/`。
...
- **任务一致性**：消费者完成业务落库后再提交 Kafka offset；任务状态更新使用条件更新，避免旧消息覆盖新状态。系统按 at-least-once 语义运行，并通过任务状态和业务幂等降低重复消费影响。
- **阶段恢复**：每个 ASR 分段独立保存状态和转写结果，重试任务只处理失败片段，避免从头重复调用外部服务。
- **混合检索**：向量召回处理语义相似问题，BM25 召回专有名词和精确关键词，RRF 在统一排名空间中融合两路结果。
- **外部服务治理**：对高成本接口做 Redis 原子限流，并记录调用、失败、重试和任务阶段指标，便于区分业务错误与 Provider 错误。
