...
func TestVideoAgentRunAgentRunsPlannerToolAndPersistsAnswer(t *testing.T) {
	...
	chatSvc := NewChatServiceWithDependencies(repos, retriever, ChatConfig{TopK: 5, CandidateK: 5, MinScore: 0.3}, ChatDependencies{})
	agent := NewVideoAgentService(chatSvc)

	result, err := agent.RunAgent(context.Background(), VideoAgentLoopRequest{
	...
	if len(messages) != 2 || messages[1].Content != result.Answer || messages[1].RetrievalSnapshot == nil {
	...
	if snapshot.Template != string(VideoAgentLoopTemplate) || traceTools(snapshot.Trace) != "search_transcript|build_cited_answer" {
	...
	for _, call := range execution.ToolCalls {
		...
		if call.CallKind == model.AgentCallKindPlannerLLM {
			plannerCalls++
			if call.ToolName != videoAgentLoopPlannerCall || call.UsageSource != model.AgentCallUsageEstimated || !call.TokenEstimated || call.PromptTokens <= 0 || call.Status != model.AgentToolCallStatusCompleted {
				t.Fatalf("planner call metadata = %+v", call)
			}
		}
	}
	if plannerCalls != 2 || execution.Run.ToolCallsUsed != 2 || execution.Run.LLMCallsUsed != 3 {
...
func TestVideoAgentResearchResumePersistsOneExchangePerRun(t *testing.T) {
	...
	req := VideoAgentLoopRequest{UserID: 7, SessionID: session.ID, Goal: "请研究 owner 校验", TopK: 1, RunID: "research-exchange-idempotency"}
	first, err := agent.RunAgent(context.Background(), req, &fakeEmbeddingClient{dim: 3}, &scriptedChatClient{responses: []string{
	...
	resumed, err := agent.ResumeAgent(context.Background(), 7, req.RunID, &fakeEmbeddingClient{dim: 3}, &scriptedChatClient{}, profile)
	if err != nil {
	...
	repeated, err := agent.RunAgent(context.Background(), VideoAgentLoopRequest{
		UserID: 7, SessionID: session.ID, Goal: req.Goal, TopK: 1, RunID: req.RunID,
		Policy: VideoAgentLoopPolicy{MaxSteps: 0, MaxReplans: -1},
	}, &fakeEmbeddingClient{dim: 3}, &scriptedChatClient{}, profile)
	if err != nil {
		t.Fatalf("repeated RunAgent() error = %v", err)
	}
...
func TestVideoAgentResearchUsesFrozenPolicyWhenCurrentPolicyIsInvalid(t *testing.T) {
	...
	agent := NewVideoAgentService(chatSvc)
	profile := ai.Profile{EmbeddingModel: "embed", LLMModel: "chat-model"}
	frozenPolicy, frozenBudget := loopAgentPolicy(1, VideoAgentLoopPolicy{MaxSteps: 3, MaxReplans: 1})
	const runID = "research-frozen-policy"
	created, err := agent.ensureAgentRun(context.Background(), runID, 7, session, "请研究 owner 校验", string(VideoAgentLoopTemplate), "default", profile, frozenPolicy, frozenBudget)
	if err != nil {
	...
	if err != nil {
		t.Fatalf("RunAgent() with invalid current policy error = %v", err)
	}
	if result.Answer == "" {
