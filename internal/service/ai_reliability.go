...
const (
	...
	// 5s 内由 caller 重试（admission 协同 spec 06）。本会话拍板子决策（见 spec 06
	// Further Notes "本会话拍板的子决策" 1）：5s 是普通 LLM 请求重试等待的合理上限，
	// 超过则用户体验已显著劣化，不如给降级答案。未经真实流量标定，长期可调。
	admissionRetryAfterCutoff = 5
)

// 降级触发计数器（进程内，长期运行需接 metrics 落盘；本 spec 只保证计数可观测、
// 验收命令能跑出真实数字，spec 第 127 行"不许估算"）。违规触发次数留 __（长期采集）。
var (
...
// degraded 标志（决策记录第 10 节拍板：API 响应带 degraded:true）。
...
// 超阈值触发档2（spec 06 Implementation Decisions: admission 协同）。本函数只判定"给定
// 失败是否该降级"，重试由 Ask 层负责。
func shouldTriggerLLMDegradation(policy ExecutionPolicy, llmErr error) bool {
	...
	// spec 06）。只有超阈值才触发档2（不再等恢复，给降级答案）。非 admission 的普通
	// LLM 错误（超时/5xx/网络）→ 直接触发档2功能性降级（决策记录第 6 节稀缺点）。
...
// 同时覆盖两类携带 RetryAfter 的错误（spec 06 admission 协同一致性）：
...
// 避免 429 短 RetryAfter 误触发档2（应为重试路径，spec 06 user story 7）。
...
// 复用 rag_expand.go 的 Fallbacks 范式（spec 06 Implementation Decisions）：
