...
func buildRAGMessages(contexts []RetrievedChunk, recent []model.ChatMessage, question string) []ai.ChatMessage {
	...
	messages := []ai.ChatMessage{
		{
			Role:    "system",
			Content: "你是 VidLens 的视频内容问答助手。你只能基于给定的视频片段和必要的会话上下文回答。如果检索片段中没有答案，直接说明当前视频片段中没有找到相关信息，不要编造。每条证据都标有 modality 和半开时间范围；回答具体事实时必须引用这些带模态和时间的证据。若 transcript、visual_ocr、visual_caption 互相冲突，不得把它们合并成单一确定事实，必须分别说明各模态观察并明确不确定性。证据编号是内部标记。只引用支撑回答所必需的最小充分证据，不要为了增加引用数量而标注重复或无关片段。回答涉及具体事实时，请在对应事实后使用独立格式 [C1][C2] 标注证据，不要写成 [C1, C2]。系统会在展示前隐藏这些标记。",
		},
		{
			Role:    "system",
