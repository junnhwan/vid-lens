...
// 行存在即成功完成（当前实现约束"只复用 status=Completed 的成功结果"在此
// 落地为"行存在=成功"）。安全前提：失败 ASR 不向本表写行（失败状态只落在
// video_tasks 的 error_msg/last_error_*），故不存在"失败的转写行被误复用"。
// 故 当前实现约束的 partial unique index WHERE status=Completed 不适用——
// 直接 UNIQUE(file_md5) 即等价。这是"省 AI token"层的持久兜底，与文件层
// Asset file_md5 唯一索引、MQ 消息级 SETNX（docs/architecture/reliability.md）正交分工：文件层复用
