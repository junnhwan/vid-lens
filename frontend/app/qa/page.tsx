    ...
    <AppShell>
      <div className="flex-1 overflow-y-auto scroll-thin">
        ...
        <div className="px-8 pb-10 max-w-3xl">
          ...
          {loading && <p className="text-[13px] text-stone-400">加载中…</p>}
            ...
            <>
              <section className="mb-8 ui-fade-in">
                <h2 className="text-[12px] uppercase tracking-wider text-stone-400 mb-3 flex items-center gap-2">
                  ...
                  单视频问答
                  <span className="text-stone-300 font-normal">· 仅本视频转写</span>
                </h2>
                <div className="space-y-2">
                  {videos.length === 0 ? (
                    <p className="text-[13px] text-stone-400 py-4">暂无已完成转写的视频，请先在视频库上传并等待转写完成。</p>
                  ) : videos.map(v => (
                    <Link
                      key={v.id}
                      href={`/chat/${v.id}`}
                      className="flex items-center gap-4 p-4 rounded-xl border border-stone-200 bg-white ui-card-hover"
                    >
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-amber-200 to-orange-300 flex items-center justify-center text-white text-[11px] font-bold shrink-0">
                        1V
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-stone-900 truncate">{taskTitle(v)}</div>
                        <div className="text-[11px] text-stone-400">任务 #{v.id} · 严格 RAG · 引用可追溯</div>
                      </div>
              ...
              <section className="ui-fade-in">
                <h2 className="text-[12px] uppercase tracking-wider text-stone-400 mb-3 flex items-center gap-2">
                  ...
                  知识库问答
                  <span className="text-stone-300 font-normal">· 跨视频检索</span>
                </h2>
                <div className="space-y-2">
                  {kbFiltered.length === 0 ? (
                    <p className="text-[13px] text-stone-400 py-4">暂无知识库，请先在知识库页创建并添加成员视频。</p>
                  ) : kbFiltered.map(k => (
                    <Link
                      key={k.id}
                      href={`/kb/${k.id}`}
                      className="flex items-center gap-4 p-4 rounded-xl border border-violet-200 bg-violet-50/30 ui-card-hover"
                    >
                      <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-violet-300 to-indigo-400 flex items-center justify-center text-white text-[11px] font-bold shrink-0">
                        KB
                      </div>
                      <div className="flex-1 min-w-0">
