...
export default function QaRecentShortcuts() {
  ...
  return (
    <div className="px-4 pt-2 pb-2 border-t border-stone-200 mt-2">
      <div className="text-[10px] uppercase tracking-wider text-stone-400 mb-2 px-1">最近可问答</div>
      {tasks.length > 0 && (
        <>
          ...
          <ul className="space-y-0.5 mb-2">
            {tasks.map(v => (
              <li key={v.id}>
                <Link
                  href={`/chat/${v.id}`}
                  className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-[12px] text-stone-700 hover:bg-amber-50 ui-row-hover"
                >
      ...
      {kbs.length > 0 && (
        <>
          <div className="text-[10px] text-violet-700/80 px-2 mb-1">知识库</div>
          <ul className="space-y-0.5">
            {kbs.map(k => (
              <li key={k.id}>
                <Link
                  href={`/kb/${k.id}`}
                  className="flex items-center gap-2 px-2 py-1.5 rounded-lg text-[12px] text-stone-700 hover:bg-violet-50 ui-row-hover"
                >
