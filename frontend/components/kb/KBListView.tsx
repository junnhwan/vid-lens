    ...
    <AppShell>
      <PageHero
        ...
        actions={!isDemo ? (
          <button
            onClick={onCreate}
            className="h-9 px-4 rounded-lg bg-stone-900 text-white text-[12px] flex items-center gap-1.5 ui-btn-lift shrink-0"
          >
      ...
      <main className="flex-1 overflow-y-auto px-8 pb-24">
        {error && (
          <div className="py-4 px-4 mb-4 text-[13px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg">
            {error}
        ...
        {loading ? (
          ...
          <div className="py-20 text-center ui-fade-in">
            ...
            {!isDemo && (
              <button
                onClick={onCreate}
                className="mt-4 h-9 px-4 rounded-lg bg-stone-900 text-white text-[12px] inline-flex items-center gap-1.5 ui-btn-lift"
              >
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {kbs.map((kb, i) => (
              <article
                ...
                className="bg-white border border-stone-200 rounded-xl p-5 ui-card-hover ui-fade-in"
                style={{ animationDelay: `${i * 60}ms` }}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="text-[17px] font-semibold text-stone-900 ui-serif truncate">{kb.name}</h3>
                  ...
                  {!isDemo && (
                    <button
                      ...
                      className="w-8 h-8 rounded-lg border border-stone-200 flex items-center justify-center text-stone-400 hover:text-red-600 hover:border-red-200 transition-colors shrink-0"
                    >
                ...
                <div className="flex gap-2 mt-4 pt-4 border-t border-stone-100">
                  {!isDemo && (
                    <button
                      onClick={() => onManage(kb)}
                      className="h-8 px-3 rounded-lg border border-stone-200 text-[11px] text-stone-600 hover:bg-stone-50 ui-btn-lift"
                    >
                      管理成员
                    </button>
                  )}
                  <Link
                    href={`/kb/${kb.id}`}
                    className="h-8 px-4 rounded-lg bg-stone-900 text-white text-[11px] flex items-center gap-1.5 ui-btn-lift ml-auto"
                  >
