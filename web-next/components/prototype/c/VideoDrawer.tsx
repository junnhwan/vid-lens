...
export function VideoDetailModal({ task: rawTask, onClose }: Props) {
  ...
  return (
    <>
      <div className="fixed inset-0 bg-stone-900/45 z-40 backdrop-blur-sm proto-backdrop" onClick={onClose} />
      <div
        className="fixed inset-4 sm:inset-8 md:inset-y-10 md:inset-x-[12%] lg:inset-x-[18%] z-50 flex flex-col bg-[#faf8f5] rounded-2xl border border-stone-200 shadow-2xl overflow-hidden proto-modal-in"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="shrink-0 flex border-b border-stone-200">
          ...
          <div className="flex-1 min-w-0 px-5 py-4 flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <h2 id="video-detail-title" className="text-[20px] sm:text-[22px] font-semibold text-stone-900 leading-tight proto-serif truncate mt-0.5">
                {taskTitle(task)}
              </h2>
            ...
            <button
              onClick={onClose}
              className="w-9 h-9 rounded-full border border-stone-200 bg-white flex items-center justify-center text-stone-500 hover:text-stone-800 hover:bg-stone-50 shrink-0 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        {/* Tab */}
        <div className="shrink-0 flex gap-1 px-5 pt-3 border-b border-stone-100 bg-white/60">
          {tabs.filter(t => t.show).map(t => (
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-2.5 text-[13px] border-b-2 -mb-px transition-colors ${
                tab === t.key
                  ? 'border-amber-600 text-stone-900 font-medium'
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-6 py-5">
          {tab === 'summary' && task.summary?.content && (
            <article className="max-w-3xl proto-fade-in">
              ...
              <p className="text-[11px] text-stone-400 mt-4">模型：{task.summary.model_name || '—'}</p>
            </article>
          )}
          ...
          {tab === 'transcript' && task.transcription?.content && (
            <article className="max-w-3xl proto-fade-in">
              <div className="rounded-xl border border-stone-200 bg-white p-5 sm:p-6">
                <pre className="text-[13px] sm:text-[14px] text-stone-700 leading-[1.85] whitespace-pre-wrap font-sans">
          ...
          {tab === 'progress' && (
            <div className="max-w-lg space-y-4 proto-fade-in">
              {phases.map(p => (
                <div key={p.label}>
                  <div className="flex justify-between text-[12px] mb-1.5">
                    ...
                    <span className="text-stone-400 tabular-nums">{p.pct}%</span>
                  </div>
                  <div className="h-2 rounded-full bg-stone-200 overflow-hidden">
                    <div
                      className={`h-full rounded-full proto-progress-bar ${
                        p.state === 'done' ? 'bg-emerald-500' : p.state === 'running' ? 'bg-amber-500' : 'bg-stone-300'
                      }`}
                      style={{ width: `${p.pct}%` }}
                    />
                  </div>
              ...
              {!task.has_transcription && (
                <p className="text-[13px] text-stone-500 italic pt-2">转写完成后，摘要与全文将出现在上方 Tab 中。</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="shrink-0 px-6 py-4 border-t border-stone-200 bg-white flex flex-wrap items-center gap-3">
          {canChat ? (
            <Link
              href={`/prototype/chat/${task.id}`}
              className="h-10 px-5 rounded-xl bg-stone-900 text-white text-[13px] font-medium inline-flex items-center gap-2 proto-btn-lift"
            >
          ...
          <Link
            href="/prototype/kb"
            className="h-10 px-4 rounded-xl border border-stone-300 text-stone-600 text-[13px] inline-flex items-center gap-1.5 hover:bg-stone-50 proto-btn-lift"
          >
            <BookOpen className="w-3.5 h-3.5" />加入知识库
          </Link>
          <button onClick={onClose} className="ml-auto text-[12px] text-stone-400 hover:text-stone-600">
            Esc 关闭
          </button>
        </div>
      </div>
...
function SummaryBody({ content }: { content: string }) {
  ...
  return (
    <div className="rounded-xl border border-stone-200 bg-white p-5 sm:p-6 space-y-3">
      {lines.map((line, i) => {
        if (line.startsWith('## ')) {
          return <h3 key={i} className="text-[16px] font-semibold text-stone-900 proto-serif pt-2 first:pt-0">{line.slice(3)}</h3>
        }
        if (line.startsWith('### ')) {
          return <h4 key={i} className="text-[14px] font-medium text-stone-800 mt-2">{line.slice(4)}</h4>
        }
        if (line.startsWith('- ')) {
          return <li key={i} className="text-[14px] text-stone-700 leading-relaxed ml-4 list-disc">{line.slice(2)}</li>
        }
        if (line.trim() === '') return <div key={i} className="h-1" />
        return <p key={i} className="text-[14px] text-stone-700 leading-relaxed">{line}</p>
      })}
    </div>
  )
}
...
export function GalleryCard({ task, selected, onClick }: { task: VideoTask; selected?: boolean; onClick: () => void }) {
  ...
  return (
    <button
      onClick={onClick}
      className={`group text-left rounded-xl overflow-hidden border bg-white transition-all duration-200 proto-card-hover ${
        selected ? 'border-amber-500 ring-2 ring-amber-500/25 shadow-md' : 'border-stone-200 hover:border-stone-300'
      }`}
    >
          ...
          {phases.map(p => (
            <span
              ...
              className={`text-[9px] px-1.5 py-0.5 rounded ${
                p.state === 'done' ? 'bg-emerald-50 text-emerald-700'
                : p.state === 'running' ? 'bg-amber-50 text-amber-700'
...
export function TableThumb({ taskId, onClick }: { taskId: number; onClick: (e: React.MouseEvent) => void }) {
  return (
    <button
      onClick={onClick}
      className="w-16 h-10 rounded-md overflow-hidden shrink-0 proto-card-hover ring-0 hover:ring-2 hover:ring-amber-400/50 transition-all"
      title="查看转写与摘要"
    >
