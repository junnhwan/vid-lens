...
func parseEvalFlags(args []string) (evalOptions, error) {
	...
	flags.StringVar(&opts.casesPath, "cases", "docs-private/eval/rag-quant-cases.yaml", "RAG eval cases YAML path")
	flags.StringVar(&opts.outputPath, "output", "artifacts/eval/rag-results.md", "local Markdown output path")
	...
	flags.StringVar(&opts.sealedAccessRegistry, "sealed-access-registry", "artifacts/eval/sealed-access-registry.jsonl", "append-only sealed test access registry")
	flags.StringVar(&opts.experimentRegistry, "experiment-registry", "docs-private/eval/experiment-registry.yaml", "preregistered experiment registry")
