...
长期记忆为 Chat 和 Agent 提供受授权的偏好上下文，不选择工具、不验证视频事实，也不控制 Agent 循环。
...
自动抽取只写 `user` scope。视频/知识库写入接口接受 `verified_claim`、`user_confirmation`、`manual` 来源类型；模型答案不自动写入事实记忆，视频事实由视频证据链路单独管理。
...
不同维度共存。同一维度的新明确偏好替代当前值：原 item 转为 `withdrawn`，版本递增并追加 `superseded` 事件，新值成为 active。来源消息 ID 用于判断先后，迟到的来源不能覆盖新选择或 tombstone。同一来源重放复用既有 item，不复活已经撤回或删除的记录；用户通过更新的消息重新明确表达偏好可以建立新记录。

普通非结构化记忆仍保留原有冲突机制：同一 owner/scope/kind 的不同内容成组标记 conflicted，不静默挑选一个值。

`response_preference` 仅对已知规范化文本进行确定性处理，保留来源、版本与变更事件。不能确定映射的内容进入治理列表，不猜测转换，也不参与自动偏好召回。撤回或删除的记录不恢复为 active。
...
用户偏好及会话覆盖使用 `expected_version` 乐观并发控制，版本不匹配返回 HTTP 409。策略更新和 `agent_memory_policy_events` 在同一事务提交。审计记录 actor、目标、前后值、版本、effective 状态和时间，不保存问题或回答。
...
来源为用户消息的结构化偏好在读取时再次校验消息 role、owner 和所属会话存在；源消息或会话删除后不进入 prompt。Snapshot 在查询前校验 scope 权限，查询固定 user/scope，随后应用 top-k、字符和近似 token 上限。失效、撤回、删除、无来源或敏感记录被排除，普通冲突项成组处理。

偏好只影响表达和问题理解，当前视频证据优先。Memory 不作为引用证据。Agent planner 和最终回答只使用服务端注入的可信 snapshot，工具参数不能自行提供 `memory_context`。持久化 Agent 记录只保存 snapshot schema/version/memory ids，不保存 snapshot 正文或 source ref；identity 不能恢复已经删除的内容。
...
生产 capture 使用 PostgreSQL outbox 和后台 worker。进程内 `AsyncMemoryWriter.Enqueue` 是非阻塞 best-effort 接口，不作为生产 capture 的可靠性边界；持久 worker 调用 writer 的写入路径完成关系持久化。
...
5. 使用 lease token CAS 完成任务；失败采用有限退避，最多 5 次尝试，到期租约可被回收。
