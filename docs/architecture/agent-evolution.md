...
产品只有 `chat` 与 `agent` 两种执行模式。知识库是检索范围，当前只提供 Chat；Agent 仅支持单视频。原模板、research 和固定漏斗不再对应独立在线引擎。
...
最终回答不再经过 Inspector 或 Claim 账本。工具参数严格校验，引用在执行与观察边界根据已观察证据 canonicalize，拒绝跨视频或未观察引用。公开引用保留真实来源、模态和时间范围；没有依据时不生成引用。
