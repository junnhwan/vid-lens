...
> 对分析消费使用带 owner 校验和 WatchDog 续租的 Redis 分布式锁，并结合 PostgreSQL processing lease、资产唯一约束和业务幂等降低同一视频并发重复处理风险；分片合并按完整文件 MD5 使用 Redis 锁串行化。

当前分片上传确实使用 Redis Set 保存临时进度，并使用 Redis 锁保护同一文件 MD5 的合并；不要把它描述成 PostgreSQL durable upload session。
...
Redis merge lock
└── vidlens:merge:<file_md5>：串行化同一文件的合并执行
```
...
> 我把并发控制分成两类：Redis owner lock 按内容指纹降低并发分析，PostgreSQL processing lease 决定具体 task/job 的执行所有权；分片 merge 则使用按文件 MD5 的 Redis 锁避免并发合并。`video_assets` 唯一约束仍是最终资产去重的数据库兜底，不能说 Redis 锁保证 exactly-once。

## 10. 不要这么说

- 不要说 Redis lock 能让分片合并 exactly-once；它只减少同一 MD5 的并发合并。
- 不要把 Redis Set 的临时已上传片号描述成 durable upload session。
- 不要说使用 Redisson。
- 不要说 MD5 是安全哈希或绝不会冲突。
