...
: "${WEB_NEXT_HEALTH_URL:?WEB_NEXT_HEALTH_URL is required}"
...
web_next_dir="$deploy_dir/web-next"
staging_dir="$deploy_dir/web-next.new-$stamp"
old_dir="$deploy_dir/web-next.old-$stamp"
backup_dir="$deploy_dir/.logs/deploy-backups/$stamp"

[ -f "$tmp_dir/web-next-src.tar.gz" ] || die "missing web-next artifact: $tmp_dir/web-next-src.tar.gz"
[ -s "$tmp_dir/web-next-src.tar.gz" ] || die "empty web-next artifact: $tmp_dir/web-next-src.tar.gz"
...
rollback_on_error() {
  ...
  warn "Deployment failed at line $line; restoring previous web-next"
  if [ -d "$old_dir" ]; then
    rm -rf -- "$web_next_dir"
    mv "$old_dir" "$web_next_dir"
  fi
  rm -rf -- "$staging_dir"
...
mkdir -p "$staging_dir"
tar -xzf "$tmp_dir/web-next-src.tar.gz" -C "$staging_dir"
(
  cd "$staging_dir"
  npm ci --no-audit --no-fund
  VIDLENS_API_BASE="$VIDLENS_API_BASE" npm run build
)
[ -d "$staging_dir/.next" ] || die "web-next build did not produce .next"

# Record a lightweight backup of the current release metadata for forensics.
mkdir -p "$backup_dir"
if [ -d "$web_next_dir" ]; then
  cp -p "$web_next_dir/package.json" "$backup_dir/" 2>/dev/null || true
  cp -p "$web_next_dir/package-lock.json" "$backup_dir/" 2>/dev/null || true
  if [ -d "$web_next_dir/.next" ]; then
    tar -czf "$backup_dir/web-next-build.tar.gz" -C "$web_next_dir" .next 2>/dev/null || true
  fi
fi
...
if [ -d "$web_next_dir" ]; then
  mv "$web_next_dir" "$old_dir"
fi
mv "$staging_dir" "$web_next_dir"

systemctl restart vidlens-web
systemctl is-active --quiet vidlens-web
curl -fsS --retry 10 --retry-delay 2 --retry-connrefused "$WEB_NEXT_HEALTH_URL"

activation_started=0
trap - ERR

if [ -d "$old_dir" ] && ! rm -rf -- "$old_dir"; then
  warn "Deployment succeeded but previous web-next directory could not be removed: $old_dir"
fi

log "Deployed web-next $sha (API base $VIDLENS_API_BASE); backup saved to $backup_dir"